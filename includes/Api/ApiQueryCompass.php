...
class ApiQueryCompass extends ApiQueryBase {
	...
	public function execute(): void {
		...
		foreach ( $res as $row ) {
			...
			if ( in_array( 'description', $prop ) ) {
				...
				$wiki['description'] = $remoteWiki->getExtraFieldData(
					'compass-description',
			...
			switch ( true ) {
				...
				case $row->wiki_inactive_exempt:
					...
					if ( in_array( 'exemptreason', $prop ) ) {
						...
						$reason = array_flip( $options )[$row->wiki_inactive_exempt_reason] ?? null;
	...
	protected function getExamplesMessages(): array {
		return [
			'action=query&list=compass' => 'apihelp-query+compass-example'
		];
	}
}
